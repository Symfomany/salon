       ...
       //$sheet =  $reader->first();
        foreach($resultat->get() as $sheet){
            ...
            foreach($sheet as $key => $row){
                  ...
                  if(!empty($row) && !in_array($row, ["[SERVICE GÉNÉRAUX]", "[MAINTENANCE ET ENTRETIEN DES LOCAUX]", "[FINANCES & SOCIAL]", "[CONSOMMABLES]", "[VÉHICULES]"])){

                       if(!preg_match("/^[\[\]A-Z0-9\'ÁÀÂÄÃÅÇÉÈÊËÍÌÎÏÑÓÒÔÖÕÚÙÛÜÝŸÆŒ& ]{3,}+$/", $row)
                       && strlen($row) > 2){ 
                           // is not a provider
                            $clef = $row;
                        }
                  ...
                  } else if(in_array($row, ["[SERVICE GÉNÉRAUX]", "[MAINTENANCE ET ENTRETIEN DES LOCAUX]", "[FINANCES & SOCIAL]", "[CONSOMMABLES]", "[VÉHICULES]"])){
                      dump($row);
                  }
