    ...
    */
    public function getAccordOne($id){
       ...
       //$sheet =  $reader->first();
        foreach($resultat->get() as $sheet){
            ...
            foreach($sheet as $key => $row){
                  ...
                  if(!empty($row)){
                    ...
                    if(!preg_match('/^[A-Z0-9 ]+$/', $row)){ // is not a provider
                        $clef = $row;
                    }
                    if($row != $clef){
                        $tab[$clef][] = $row;
        ...
        // exit(dump($resultat->skip(6)->take(6)));
        $resultatOne = $resultat->take(17);
        $resultatTwo = $resultat>slice(17, 6);
        $resultatThree = $resultat>slice(23, 7);
        $resultatFour = $resultat>slice(30, 5);
        $resultatFive = $resultat>slice(35, 11);
