       ...
       //$sheet =  $reader->first();
        foreach($resultat->get() as $sheet){
            ...
            foreach($sheet as $key => $row){
                  ...
                  if(!empty($row) && if(in_array($row, ["[SERVICE GÉNÉRAUX]", "[MAINTENANCE ET ENTRETIEN DES LOCAUX]", "[FINANCES & SOCIAL]", "[CONSOMMABLES]", "[VÉHICULES]"]){){
                    ...
                    //   if($row == "[SERVICE GÉNÉRAUX]"){
                    //       $resultatOne[] = $row;
                    //   }else if($row == "[MAINTENANCE ET ENTRETIEN DES LOCAUX]"){
                    //       $resultatTwo = $row;
