    ...
    */
    public function getAccordOne($id){
       ...
       //$sheet =  $reader->first();
        foreach($resultat->get() as $sheet){
            ...
            foreach($sheet as $key => $row){
                  ...
                  if(!empty($row)){
                    ...
                    if(!preg_match('/^[A-Z0-9ÁÀÂÄÃÅÇÉÈÊËÍÌÎÏÑÓÒÔÖÕÚÙÛÜÝŸÆŒ ]+$/', $row)){ // is not a provider
                        $clef = $row;
                    }
        ...
        $resultatTwo = $resultat->slice(16, 6)->toArray();
        $resultatThree = $resultat->slice(22, 7)->toArray();
        $resultatFour = $resultat->slice(29, 5)->toArray();
        $resultatFive = $resultat->slice(34, 11)->toArray();
